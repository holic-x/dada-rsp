...
<title>哒哒报表平台用户登录</title>

<!-- 引入轮播图相关的css样式 -->
<link rel="stylesheet" href="${pageContext.request.contextPath }/resource/custom/background/carousel.css" />
<!-- 引入相关的css、js文件 -->
			...
			<div class="col-md-4 col-md-offset-4">
				<div class="login-panel panel panel-default">
					<div class="panel-heading">
						<h3 class="panel-title" style="text-align: center;">哒哒报表平台用户登录</h3>
					</div>
					<div class="panel-body">
						<form role="form" id="submitForm">
							<fieldset>
								...
								<div class="form-group">
									...
									<input class="form-control" type="text" placeholder="请输入登录名称/注册邮箱" id="loginAccount" name="loginAccount">
								</div>
								<div class="form-group">
						...
						<div class="form-group">
							<!-- <label class="col-sm-4 control-label">信息注册</label> -->
							<div class="col-sm-12">
								<button class="btn btn-sm" id="categoryRegister">注册机构</button>
								&nbsp;
								<button class="btn btn-sm" id="userRegister">注册用户</button>
								&nbsp;
								<button class="btn btn-sm" id="test">平台试用</button>
							</div>
						</div>
					</div>
				</div>
	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$("#loginPassword").blur(function(){
				// $("#submitForm").bootstrapValidator(checkEditFrom());
				var validator = $('#submitForm').data("bootstrapValidator").validateField('loginPassword'); //获取validator对象
			});
			
			// categoryCode失去焦点触发验证
			$("#categoryCode").blur(function(){
				// $("#submitForm").bootstrapValidator(checkEditFrom());
				var validator = $('#submitForm').data("bootstrapValidator").validateField('categoryCode'); //获取validator对象
			});
			
			// 点击相应的按钮跳转页面
			$("#categoryRegister").click(function(){
				$('#submitForm').data('bootstrapValidator',null);
				// $("#submitForm").data('bootstrapValidator').destroy();
				var url = '${pageContext.request.contextPath }/sso/page/register?url=category_register';
				window.location.href = url;
			});
			$("#userRegister").click(function(){
				$("#submitForm").data('bootstrapValidator').destroy();
				var url = '${pageContext.request.contextPath }/sso/page/register?url=user_register';
				window.location.href = url;
			});
			$("#test").click(function(){
				...
				success : function(responseData) {
					...
					if (result_code == "1") {
						...
						if(result=='success'){
							...
							// 获取当前用户的机构编码，如果为dada默认是后台管理员登录
							var categoryCode = $("#categoryCode").val();
							if(categoryCode=='dada'){
								var url = "http://localhost:8080/manager/page/index";
								window.location.href = url;
							}else{
								// 其余机构编码默认是前台报表平台
								// var url = "http://localhost:8082/portal/page/index?url=adminIndex";
								var url = "http://localhost:8082/portal/index/home";
								window.location.href = url;
							}
