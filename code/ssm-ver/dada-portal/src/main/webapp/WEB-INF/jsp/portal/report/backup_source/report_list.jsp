...
<body>
		...
		$(document).ready(function() {
			...
			ajax : function(data, callback,settings) {
				...
				$.ajax({
					...
					url : '${pageContext.request.contextPath}/portal/report/manager/listUreportFileByPage',
					type : "POST",
	...
	$('#dataTables-example').on('click','button#delrow',function(even) {
		...
		if (confirm("确定删除该数据信息?") == false) {
			...
			$.ajax({
				url : '${pageContext.request.contextPath }/portal/report/manager/deleteUreportData',
				type : 'POST',
				contentType : 'application/json; charset=UTF-8',
				dataType : 'json',
	...
	$('#dataTables-example').on('click','button#editrow',function(even) {
		...
		var data = $("#dataTables-example").DataTable().row($(this).parents('tr')).data();
		// 封装选择的数据行
		var url = "${pageContext.request.contextPath }/portal/report/manager/updateUreportFileUI?fileId="+ data.fileId;
		window.location.href = url;
	});
