...
<body>
	...
	<script>
		$(document).ready(function() {
			...
			ajax : function(data, callback,settings) {
				...
				//ajax请求数据
				$.ajax({
					url : contentPath + '/system/listRoleInfoByPage',
					type : "POST",
	...
	$("#btn_add").click(function() {
		...
		// 跳转到用户添加页面
		var url = "${pageContext.request.contextPath }/portal/page/other?url=portal/system/roleInfo/role_add";
	...
	$('#dataTables-example').on('click','button#delrow',function(even) {
		...
		if (confirm("确定删除该数据信息?") == false) {
			...
			$.ajax({
				url : '${pageContext.request.contextPath }/portal/system/deleteRoleInfo',
				type : 'POST',
				contentType : 'application/json; charset=UTF-8',
				dataType : 'json',
	...
	$('#dataTables-example').on('click','button#editrow',function(even) {
		...
		var data = $("#dataTables-example").DataTable().row($(this).parents('tr')).data();
		// 封装选择的数据行
		var url = "${pageContext.request.contextPath }/portal/system/updateRoleInfoUI?roleId="+ data.roleId;
		window.location.href = url;
	});
