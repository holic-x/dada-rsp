				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
						...
						<form id="submitForm" class="form-horizontal" action="#">
							...
							<input type="hidden" id="deptId" name="deptId" value="${ureportData.deptId}" />
						...
						<!-- 报表权限设置:公开状态、可见状态  end -->
							...
							<div class="row">
								<div class="col-sm-3 col-sm-offset-4">
									...
									<!-- 提供编辑报表信息入口 -->
									<a class="btn btn-warning" href="http://localhost:8084/ureport/designer?_u=dada-mysql-custom:${ureportData.fileName }">编辑报表内容</a>
								</div>
							</div>
						</form>
					</div>
    ...
    $("#submitButton").click(function() {//非submit按钮点击后进行验证，如果是submit则无需此句直接验证
     	...
     	if (validator.isValid()) {
        	...
        	return;
         	var submitData = {
         		// 操作类型
         		// "operatorType":"report_organ", 原始指定操作类型(代码层控制)
         		"tableName":"ureport_file", // 通用指定操作表名,dao层控制
         		// 修改数据
         		"ureportFileManager":{  // 原始参数ureportFile
         			// 封装隐藏属性
    				"fileId":$("#fileId").val(),
           	...
           	$.ajax({
           		url :'${pageContext.request.contextPath}/portal/report/manager/common/updateUreportData',
               	type : 'POST',
             	...
             	async : false,
              	...
              	success : function(responseData) {
              		...
              		if (result_code == "1") {
               			// 返回状态码为1,数据访问成功
                 		alert('服务器反馈^_^:报表基本信息修改成功,正在返回页面.');
