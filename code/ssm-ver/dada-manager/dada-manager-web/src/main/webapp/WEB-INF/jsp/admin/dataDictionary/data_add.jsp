...
<body onload="loadData()">
										...
										<div id="dataCodeGroup" class="form-group">
											...
											<div class="col-sm-9">
												<input type="text" id="dataCode" name="dataCode"
													class="form-control input-sm" placeholder="请输入数据编码" />
											</div>
										</div>
									</div>
										...
										<div class="form-group">
											...
											<div class="col-sm-9">
												<input type="text" id="dataName" name="dataName"
													class="form-control input-sm" placeholder="请输入数据名称" />
											</div>
										</div>
									</div>
										...
										<div class="form-group">
											...
											<div class="col-sm-9">
												<textarea class="form-control" rows="3" cols="50"
													id="dataDesc" name="dataDesc" placeholder="请填写数据备注信息"></textarea>
											</div>
										</div>
									</div>
								</div>
	...
	<script type="text/javascript">
		function checkEditFrom() {
			...
			var contentPath = "/" + location.pathname.split("/")[1];
			var validateLoginAccountPath = contentPath + "/user/validateRepeatLoginAccount";
		...
		$("#submitButton").click(function() {//非submit按钮点击后进行验证，如果是submit则无需此句直接验证
			...
			if (validator.isValid()) {
				...
				var contentPath = "/" + location.pathname.split("/")[1];
				var validateDataCodePath = contentPath + "/dataDictionary/validateRepeatDataCode";
				var dataCodeData = {
				...
				$.ajax({
					...
					success : function(responseData) {
						...
						if (responseData.data.valid == "true") {
							alert('操作反馈^_^:数据编码重复，请确认后重新操作.');
							repeatDataCode = Boolean(true);
							// 设置样式
							$('#dataCodeGroup').addClass('has-error');
				...
				if (!repeatDataCode) {
					...
					$.ajax({
						url : contentPath + '/dataDictionary/addData',
						type : 'POST',
						contentType : 'application/json; charset=UTF-8',
						// async : false,
						dataType : 'json',
