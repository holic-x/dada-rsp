	...
	<script type="text/javascript">
		...
		$("#submitButton").click(function() {//非submit按钮点击后进行验证，如果是submit则无需此句直接验证
			...
			if (validator.isValid()) {
				...
				var contentPath = "/" + location.pathname.split("/")[1];
				/* var validateRepeatPath = contentPath + "/user/validateRepeatLoginAccount/" + "?loginAccount=" + $("#loginAccount").val(); */
				...
				$.ajax({
					...
					success : function(responseData) {
						...
						if (responseData.data.valid == "true") {
							alert('操作反馈^_^:用户登录账号重复，请确认后重新操作!');
							repeatLoginAccount = Boolean(true);
							// 设置样式
							$('#loginAccountGroup').addClass('has-error');
								return;
						}
					}
				});
				...
				$.ajax({
					...
					success : function(responseData) {
						// console.log(responseData);
						if (responseData.data.valid == "true") {
							alert('操作反馈^_^:用户注册邮箱重复，请确认后重新操作!');
							repeatEmail = true;
						// 设置样式
						$('#loginAccountGroup').addClass('has-error');
							return;
				...
				// alert('loginAccount:' + repeatLoginAccount + '----repeatEmail:' + repeatEmail)
				if ((!repeatLoginAccount) && (!repeatEmail)) {
					...
					$.ajax({
						url : contentPath + '/user/addUserInfo',
						type : 'POST',
						contentType : 'application/json; charset=UTF-8',
						// async : false,
						dataType : 'json',
