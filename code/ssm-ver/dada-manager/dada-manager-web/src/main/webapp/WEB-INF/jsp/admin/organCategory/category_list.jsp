...
<body>
	...
	<div id="wrapper">
		...
		<div id="page-wrapper">
			<div class="row">
				<div class="col-lg-12">
					<h1 class="page-header">机构信息管理</h1>
				</div>
				<!-- /.col-lg-12 -->
			</div>
			<!-- /.row -->
					...
					<div class="panel panel-default">
						...
						<div class="panel-body">
							<table width="100%"
								...
								<thead>
									<tr>
										<th class="text-center">机构编号</th>
										<th class="text-center">机构编码</th>
										<th class="text-center">机构名称</th>
										<th class="text-center">机构描述</th>
	...
	<script>
		...
		$(document).ready(function() {
			...
			$('#dataTables-example').DataTable(
			{
				...
				ajax : function(data, callback,settings) {
					...
					//ajax请求数据
					$.ajax({
						url : contentPath + '/organCategory/listCategoryByPage',
						type : "POST",
		...
		$("#btn_add").click(function() {
			...
			// 跳转到机构添加页面
			var url = "${pageContext.request.contextPath }/manager/page/other?url=organCategory/category_add";
		...
		$('#dataTables-example').on('click','button#delrow',function(even) {
			...
			alert('友情提示^_^:机构删除将会导致关联数据清空,请谨慎操作.');
			if (confirm("友情提示^_^:确定删除该数据信息?") == false) {
				...
				*/
				...
				$.ajax({
					url : '${pageContext.request.contextPath }/manager/organCategory/deleteCategory',
					type : 'POST',
					contentType : 'application/json; charset=UTF-8',
					dataType : 'json',
		...
		$('#dataTables-example').on('click','button#editrow',function(even) {
			...
			var data = $("#dataTables-example").DataTable().row($(this).parents('tr')).data();
			// 封装选择的数据行
			var url = "${pageContext.request.contextPath }/manager/organCategory/updateCategoryUI?categoryId=" + data.categoryId;
			window.location.href = url;
		});
