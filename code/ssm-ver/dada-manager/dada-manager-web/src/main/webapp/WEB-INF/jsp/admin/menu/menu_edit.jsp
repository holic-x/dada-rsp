					...
					<div class="panel panel-default">
						...
						<div class="panel-body">
							...
							<form id="submitForm" class="form-horizontal" action="#">
								...
								<input type="hidden" id="delFlag" name="delFlag" value="${authorityInfo.delFlag }" />
								<!-- 菜单编码由后台指定 -->
								<input type="hidden" id="authorityCode" name="authorityCode" value="${authorityInfo.authorityCode }" />
										 ...
										 <div class="form-group">
											...
											<div class="col-sm-9">
												...
												<select class="form-control input-sm" id="authorityIcon" name="authorityIcon" >
													<option value="icon-desktop" ${authorityInfo.authorityIcon=='icon-desktop'?'selected':'' }>icon-desktop</option>
													<option value="icon-search" ${authorityInfo.authorityIcon=='icon-search'?'selected':'' }>icon-search</option>
													<option value="icon-table" ${authorityInfo.authorityIcon=='icon-table'?'selected':'' }>icon-table</option>
													<option value="icon-dashboard" ${authorityInfo.authorityIcon=='icon-dashboard'?'selected':'' }>icon-dashboard</option>
													<option value="icon-sitemap" ${authorityInfo.authorityIcon=='icon-sitemap'?'selected':'' }>icon-sitemap</option>
													<option value="icon-envelope" ${authorityInfo.authorityIcon=='icon-envelope'?'selected':'' }>icon-envelope</option>
													<option value="icon-user" ${authorityInfo.authorityIcon=='icon-user'?'selected':'' }>icon-user</option>
													<option value="icon-th-list" ${authorityInfo.authorityIcon=='icon-th-list'?'selected':'' }>icon-th-list</option>
													<option value="icon-signal" ${authorityInfo.authorityIcon=='icon-signal'?'selected':'' }>icon-signal</option>
													<option value="icon-cog" ${authorityInfo.authorityIcon=='icon-cog'?'selected':'' }>icon-cog</option>
													<option value="icon-cogs" ${authorityInfo.authorityIcon=='icon-cogs'?'selected':'' }>icon-cogs</option>
													<option value="icon-trash" ${authorityInfo.authorityIcon=='icon-trash'?'selected':'' }>icon-trash</option>
													<option value="icon-home" ${authorityInfo.authorityIcon=='icon-home'?'selected':'' }>icon-home</option>
													<option value="icon-lock" ${authorityInfo.authorityIcon=='icon-lock'?'selected':'' }>icon-lock</option>
													<option value="icon-globe" ${authorityInfo.authorityIcon=='icon-globe'?'selected':'' }>icon-globe</option>
													<option value="icon-bullhorn" ${authorityInfo.authorityIcon=='icon-bullhorn'?'selected':'' }>icon-bullhorn</option>
													<option value="icon-folder-open" ${authorityInfo.authorityIcon=='icon-folder-open'?'selected':'' }>icon-folder-open</option>
													<option value="icon-comment" ${authorityInfo.authorityIcon=='icon-comment'?'selected':'' }>icon-comment</option>
													<option value="icon-comments-alt" ${authorityInfo.authorityIcon=='icon-comments-alt'?'selected':'' }>icon-comments-alt</option>
													<option value="icon-leaf" ${authorityInfo.authorityIcon=='icon-leaf'?'selected':'' }>icon-leaf</option>
													<option value="icon-tasks" ${authorityInfo.authorityIcon=='icon-tasks'?'selected':'' }>icon-tasks</option>
													<option value="icon-group" ${authorityInfo.authorityIcon=='icon-group'?'selected':'' }>icon-group</option>
												</select>
											</div>
										</div>
									</div>
										...
										<div class="form-group">
											...
											<div class="col-sm-9">
												<textarea class="form-control" rows="3" cols="50"
													...
													placeholder="请填写菜单备注信息">${authorityInfo.authorityDescr }</textarea>
											</div>
										</div>
									</div>
								</div>
			...
			if (validator.isValid()) {
				...
				*/
				...
				if (!repeatAuthorityUrl) {
					...
					// 2.请求服务器传送json菜单，修改信息
					$.ajax({
						url : contentPath + '/menu/updateAuthorityInfo',
						type : 'POST',
						contentType : 'application/json; charset=UTF-8',
						// async : false,
						dataType : 'json',
