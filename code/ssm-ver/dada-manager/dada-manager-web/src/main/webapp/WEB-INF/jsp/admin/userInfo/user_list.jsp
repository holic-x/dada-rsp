	...
	<script>
		...
		*/
		$(document).ready(function() {
			...
			*/
			// var searchParams = {};
			// searchParams.logAccount = 'hahabibu';
			var searchParams = 
			{
				"pageData": {
					"loginAccount":"hahabibu"
			...
			$('#dataTables-example').DataTable(
			{
				...
				// ajax : contentPath + '/user/listUserInfoByPage', 默认Get方式
				// 能够正常访问数据，但数据无法加载到表格中
				/* 
					ajax : {
						...
						// async : false,
						dataType : 'json',
						//  data : JSON.stringify(searchParams), 乱码
						// data : searchParams, 字符串
						data : function(request) {
							// 设置参数属性，转化为json数据
							request.loginAccount = 'hahabibu';
		...
		$('#dataTables-example').on('click','button#delrow',function(even) {
			...
			if (confirm("友情提示^_^:确定删除该用户信息?") == false) {
				return;
			} else {
			/* 得到当前对象的值 */
			...
			$.ajax({
				url : '${pageContext.request.contextPath }/manager/user/deleteUserInfos',
				type : 'POST',
				contentType : 'application/json; charset=UTF-8',
				dataType : 'json',
		...
		$('#dataTables-example').on('click','button#editrow',function(even) {
			...
			var data = $("#dataTables-example").DataTable().row($(this).parents('tr')).data();
			// 封装选择的数据行
			var url = "${pageContext.request.contextPath }/manager/user/updateUserInfoUI?userId="+data.userId;
			window.location.href = url;
		});
