 ...
 * 
 * @ClassName：Page 
 * @Description: 分页参数对象，需要分页的对象需引入
 * @author KangMiao
 * @Date 2016年7月8日 下午1:52:52
 *
 */
public class Page {
...
//				sb.append("	<li style=\"cursor:pointer;\"><a onclick=\"toTZ();\"  class=\"btn btn-mini btn-success\">跳转</a></li>\n");
			...
			if(10 != showCount && 20 != showCount && 30 != showCount && 30 != showCount && 40 != showCount && 50 != showCount
					 && 60 != showCount && 70 != showCount && 80 != showCount && 90 != showCount && 99 != showCount) {
			...
			if (showCount == 60) {
				sb.append("	<option value='60' selected>60</option>\n");
			} else {
				sb.append("	<option value='60'>60</option>\n");
			}
			if (showCount == 70) {
				sb.append("	<option value='70' selected>70</option>\n");
			} else {
				sb.append("	<option value='70'>70</option>\n");
			}
			if (showCount == 80) {
				sb.append("	<option value='80' selected>80</option>\n");
			} else {
				sb.append("	<option value='80'>80</option>\n");
			}
			if (showCount == 90) {
				sb.append("	<option value='90' selected>90</option>\n");
			} else {
				sb.append("	<option value='90'>90</option>\n");
			}
			if (showCount == 99) {
				sb.append("	<option value='99' selected>99</option>\n");
			} else {
			...
			sb.append("if(toPaggeVlue == ''){document.getElementById(\"toGoPage\").value=1;"
					+ "$(\"#toGoPage\").tips({side:1,msg:'请输入正整数',bg:'#AE81FF',time:3});return;}");
			sb.append("if(!new RegExp(/^[1-9]+[0-9]*]*$/).test(toPaggeVlue)){document.getElementById(\"toGoPage\").value=1;"
					+ "$(\"#toGoPage\").tips({side:1,msg:'请输入正整数',bg:'#AE81FF',time:3});return;}");
