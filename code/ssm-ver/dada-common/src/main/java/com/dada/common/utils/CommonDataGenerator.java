...
public class CommonDataGenerator {
	...
	static {
		String[] dept1 = {"行政总监","行政主管","行政专员","行政助理","后勤专员","后勤助理"};
		String[] dept2 = {"税务总监","税务主管","投资主管","成本主管","审计主管","会计","出纳","统计员","税务专员","税务助理"};
		String[] dept3 = {"质量计划主管","质量计划专员","质量工程师","产品质量策划员","质量检验员","质量测试员","审核员","认证工程师","供应商","采购专员","采购助理"};
		String[] dept4 = {"销售总监","销售区域经理","销售工程师","客户经理","商务经理 ","商务专员","分销主管","销售专员","销售助理"};
		String[] dept5 = {"硬件工程师","软件工程师","技术支持工程师","网络工程师","系统工程师","数据库工程师","信息安全工程师","软件架构师","测试工程师"};
		String[] dept6 = {"人力资源总监","人力资源主管","招聘主管","绩效主管","薪酬主管","员工关系主管","人力资源专员","人力资源助理","人力资源实习生"};
		// 封装部门信息
		DEPT.put("行政部", dept1);
		DEPT.put("财务部", dept2);
		DEPT.put("质量管理部", dept3);
		DEPT.put("营销部", dept4);
		DEPT.put("技术部", dept5);
		DEPT.put("人力资源部", dept6);
	}
	
	/**
	 * 随机生成部门-职位信息
	 */
	public static Map<String, String> randomDeptAndJob() {
		Map<String, String> map = new HashMap<>();
		Random random = new Random();
		int deptNo = random.nextInt(6);
		String deptName = "无所属部门";
		if(deptNo==0) {
			deptName = "行政部";
		}else if(deptNo==1) {
			deptName = "财务部";
		...
		int jobNo = random.nextInt(DEPT.get(deptName).length);
		String jobName = DEPT.get(deptName)[jobNo];
		map.put(deptName, jobName);
		return map;
	}
	 ...
	 * 根据部门随机生成职位信息
	 */
	public static String randomJob(String deptName) {
		Random random = new Random();
		int jobNo = random.nextInt(DEPT.get(deptName).length);
		return DEPT.get(deptName)[jobNo];
	}
	 ...
	 * 随机生成婚姻状态
	 */
	public static String randomMarried() {
		int married = getNum(0, 2);
		if (married == 0) {
			return "未婚";
		} else if (married == 1) {
			return "已婚";
	 ...
	 * 随机生成学历信息
	 */
	public static String randomDegree() {
		Random random = new Random();
		String[] degree = {"小学","初中","高中","大专","大学本科","研究生"};
		return degree[random.nextInt(degree.length)];	
	}
	...
	public String randomBirthday(int age) {
		...
		birthday.set(Calendar.YEAR, (int) (Math.random() * age) + y - age);
		birthday.set(Calendar.MONTH, (int) (Math.random() * 12));
		birthday.set(Calendar.DATE, (int) (Math.random() * 31));
	...
	public static double randomSalary() {
		...
		// 生成整数位
		Random random =  new Random();
		return random.nextInt(9999) + Math.floor((Math.random() * (max - min) + min) * pow) / pow;
	}
	 ...
	 * 随机生成编号(纯数字)
	 */
	public static String random10num() {
	 ...
	 * 随机生成10位字符序列(数字、字母)
	 */
	public static String random10charSeq() {
		StringBuffer sb = new StringBuffer();
	...
	public static Map<String, String> getAddress() {
		Map map = new HashMap();
		map.put("name", randomChineseName());
		map.put("sex", name_sex);
		map.put("road", randomRoad());
		map.put("tel", randomPhone());
		map.put("email", randomEmail(6, 9));
		return map;
	}
